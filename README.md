...
取り込みの時点で、実際にデコードできるかを確かめる（`AssetCache.inspect`）。

- 対応外のコーデック（VP9 / FFV1 など）は `AVAssetTrack.isDecodable` で分かる
- 途中で切れたファイルはフラグでは分からないので、末尾付近のフレームを 1 枚読んでみる

こうしないと気づく手立てが無い。AVPlayer は対応外のコーデックでも `readyToPlay` を
返して黒を流し、途中で切れたファイルでも最後まで時間を進める。`status` が `.failed` に
なることも `failedToPlayToEndTime` が飛ぶことも、手元で試した壊れ方では一度も無かった。
組み立て（`CompositionBuilder`）でも `isDecodable` を見る。別の Mac で作った
プロジェクトを開いたときのため。

テスト用の素材は `NanovidTests/Fixtures/` に置いてある。`undecodable-ffv1.mov` は
16x16 の 1 コマ、`tiny-h264.mp4` は moov を先頭にした 2 秒の H.264 で、切断のテストは
これを途中で切って作る。
...
字幕タブの「自動生成」から、言語とテンプレートを選んで実行する。再生とは関係なく
その場で走る。

進み具合は段階ごとに出す。割合だけだと、聞き取りに入る前の準備で長く 0% のまま
止まって見えるため。とくに初回は言語モデルの取り寄せに時間がかかる。
割合が測れない段階ではぐるぐるを出し、測れる段階だけ帯にする。

```
準備中 → 言語モデルを取り寄せ中 45% → 音声をまとめ中 → 聞き取り中 62% → 字幕にしています
```

聞き取りそのものは実時間より速く進む。12 秒の音声なら 1 秒かからないので、
体感の待ち時間はほとんど準備の段階にある。
...
吸着は「編集操作 > クリップを吸着させる」で入切できる（設定は端末ごとに覚える）。
ツールバーに絵柄で置くと何のアイコンか伝わりにくいので、メニューに寄せてある。
ドラッグの最中に ⌥ を押しているあいだは、その設定と逆になる。切っていてもフレーム境界へは
丸める。フレームから外れた位置は動画として意味がないため。
